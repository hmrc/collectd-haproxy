    ...
    def connect(self):
        ...
        if is_unix:
            ...
            if socket_host != "" and port != "" and separator == ":":
                ...
                collectd.error(
                    "Could not connect to socket with host %s. Check HAProxy config."
                    % self.socket_file
                )
                return
            ...
            try:
                stats.append(
                    (
                        metricname,
                        int(val),
                        {"is_resolver": True, "nameserver": resolver},
                    )
                )
            except (TypeError, ValueError):
                pass
...
def should_capture_metric(statdict, module_config):
    return (
        (
            "svname" in statdict
            and statdict["svname"].lower() in module_config["proxy_monitors"]
        )
        or (
            "pxname" in statdict
            and statdict["pxname"].lower() in module_config["proxy_monitors"]
        )
        or is_backend_server_metric(statdict)
        and "backend" in module_config["proxy_monitors"]
    )
...
def _format_plugin_instance(dimensions):
    if is_backend_server_metric(dimensions):
        return "{0}.{1}.{2}".format(
            "backend", dimensions["pxname"].lower(), dimensions["svname"]
        )
...
def _str_to_bool(val):
    ...
    if val == "true":
        ...
        collectd.warning(
            "Warning: String (%s) could not be converted to a boolean. Returning false."
            % val
        )
